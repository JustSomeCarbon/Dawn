...
Solace is built to take advantage of the different types of declarative programming paradigms such as
...
Solace is written in C and compiled with the $Clang$ compiler and compiles down to llvm byte code. Apart from this, the lexer for Solace is
...
\textbf{NOT} a  build tool available, so all library requirements should be installed independentally. Solace Relies on Clang,
Bison, and Flex. Solace is hosted on github \href{https://github.com/JustSomeCarbon/Solace/tree/main}{here} and can be cloned
to a local machine using git. After Solace is installed, it can be built by typing the following into the console while within the
$Solace/src$ project directory:
...
Once this completes, the executable $arc$ file will be available. This is the Solace compiler program.
To compile a Solace file run the following with the Solace source file to compile:

\begin{lstlisting}
 $ ./anvl solaceFileToCompile.solc
\end{lstlisting}
...
While this is a simple program we can see how Solace files are structured. After saving the source file as $justExit.arc$ we can
...
mod Main
	def main() int do
		let letter char = 'c'
		let passage string = "hello, world!"
...
for Solace are designed to be similar to other languages, such as $C$ and $Elixir$. A lot of the reserved words are defined for type declarations shown above, or the legal operators for the 
...
mod Main
	def main () int do
		let character1 char = 'h'
		let character2 char = 'i'

			// result is a string
		let concat1 string = character1 | character2

		let unfinishedString string = "ello"

			// result is a string: "hello"
		let concat2 string = character1 | unfinishedString

		let unfinishedString2 string = ", world!"

			// result is a string: "hello, world!"
		let concat3 string = concat2 | unfinishedString2
...
mod Main
	def main () int do
		...
		let raiseToPower fn = (n int, r int) int do
			if (r <= 1) then
				return n;
			end
			n * self(n, r-1);
...
mod Main
	...
	def main () int do
		...
		let tempuratures [float] = {32.7, 46.9, 53.1, 68.4, 73.1}
...
mod Main
	...
	def main () int do
		let n int = 0
		let nums [int] = {1,2,3}
		let nums2 [int] = {4,5,6,7}

		// combine lists with concat
		let combinedNums [int] = n | nums // returns {0,1,2,3}

		let finalCombinedNums [int] = combinedNums // returns {0,1,2,3,4,5,6,7}
...
mod Main
	...
	def main () do
		let playerOne PlayerCharacter = {"Echo", 14, 186.9}
